...
'''
void update_single(bool can_assign = false)
{
  ...
  while (temp != nullptr)
  {
    ...
    else if (temp->get_queue_list()->get_head() == nullptr && singleQueue->get_head() != nullptr)
    {
      if (temp->get_availableTime() < cur_time)
      {
        cur_time = temp->calculateDepartTime();
        min = temp;
      }
  ...
  if (min != nullptr)
  {
    if (min->get_queue_list()->get_head() == nullptr && singleQueue->get_head() != nullptr)
    {
      min->get_queue_list()->enqueue(singleQueue->dequeue());
      cout << "Queued a customer with free register " << min->get_ID() << endl;
    }
    else if (min->get_queue_list()->get_head() != nullptr)
    {
